...
async function getUserName(conn, jid, pushNameFromStub = '') {
    // Ignora pushNameFromStub se è 'created' o valori placeholder noti
...
export async function before(m, { conn, groupMetadata }) {
    ...
    const cleanUserId = jid.split('@')[0].replace(/:\d+$/, ''); // Pulizia LID/device ID
    ...
    const isRemove = m.messageStubType === WAMessageStubType.GROUP_PARTICIPANT_LEAVE || m.messageStubType === WAMessageStubType.GROUP_PARTICIPANT_REMOVE;
